<template>
    <div class="p-4 container mx-auto z-10">
      <!-- Search Input and Filter Icon -->
      <section class="bg-[#FFF9F9CC] max-w-7xl mx-auto px-2 lg:px-3">
        <div class="flex items-center border-2 border-red-600  rounded-lg bg-white p-3 shadow-lg">
          ...
          <input ref="inputRef"  @keyup.enter="handleEnter" type="text" v-model="query" placeholder="Search by name or location..."
      ...
      <div v-if="loading" class="text-center text-gray-600 mt-5">
        <div class="spinner"></div>
        Searching...
      </div>
  
      <div v-else-if="!location && hospitals?.length === 0"
        class="text-center text-gray-600 border rounded border-gray-100 mt-4 py-6">
      ...
      <div v-else class="p-2 lg:p-4">
        <p class="text-gray-600 text-sm mb-4">Results are automatically filtered based on your location</p>
  ...
  <div
    ...
    class="p-4 border rounded-lg shadow cursor-pointer transition-transform transform hover:scale-105"
    :class="{
  ...
  >
    <div class="flex justify-between items-center">
      ...
      <span
        class="text-xs rounded-full px-2 py-1 text-white"
        :class="{
          'bg-green-500': hospital.availability === 'available',
          'bg-red-500': hospital.availability === 'unavailable',
          'bg-blue-500': hospital.availability === 'busy',
        }"
      >
    ...
    <p class="text-sm text-gray-600 mt-2">
      {{ hospital.vicinity }}
        ...
        <!-- <div class="flex space-x-4 overflow-x-auto custom-scrollbar">
          <div v-for="hospital in hospitals" :key="hospital.place_id"
