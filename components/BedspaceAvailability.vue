<!-- <template>
      ...
      <div class="container mx-auto">
        ...
        <div v-else class="p-4">
          ...
          <div class="flex space-x-4 overflow-x-auto custom-scrollbar">
            <div @click="handleSelectedClick(hospital)" v-for="hospital in displayedHospitals" :key="hospital.place_id || hospital._id" @click="selectHospital(hospital)" class="hospital-card relative w-64 p-4 m-2 space-y-3 border rounded-lg shadow cursor-pointer transition-transform transform hover:scale-105" :class="{
  ...
  const handleSelectedClick = (hospital) => {
  ...
  // NEW: Get hospital click information
  ...
  // NEW: Handle hospital click with WebSocket integration
  const handleHospitalClick = async (hospital: any) => {
    console.log('🖱️ Hospital clicked:', hospital);
    
    // Get hospital ID (prefer _id for API hospitals, place_id for Google hospitals)
    const hospitalId = hospital._id || hospital.place_id;
    ...
    try {
      ...
      // Navigate to hospital details after successful click
      setTimeout(() => {
